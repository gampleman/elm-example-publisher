...
module.exports = function(options) {
      ...
      .then(function(templateSrc) {
        ...
        return function(output, data) {
          ...
          return fs.writeFile(path.join(options.outputDir, output), result).then(function(out) {
            ...
            return out;
          }, e => console.error('Failed to generate ' + path.join(options.outputDir, output), e));
        }
      });
  }
  ...
  function snapPicture(dir, name, output, delay, additionalOptions) {
    ...
    return captureWebsite.file(output, bigPicture, _.merge({
      inputType: 'url',
      width: options.width,
      height: options.height,
      delay: delay
    }, additionalOptions.capture)).then(function() {
        ...
        .then(function() {
          return Promise.all(elmData.map(function(ed) {
            return showTemplate(path.join(ed.name, 'index.html'), _.merge({
              allExamples: elmData, width: options.width, height: options.height, package: elmPackage, custom: options.templateData
            }, ed));
          }))
        });
    }));
  ...
  return Promise.all([glob(options.inputDir + '/**/*.elm'), glob(options.inputDir + options.exclude)])
    ...
    .then(filterFiles)
    .then(async function(elmFilesWithMain) {
      const previews = await compileAndRenderPreviews(elmFilesWithMain)
      const pages = await renderPages(elmFilesWithMain);
